...
public class CommonIntentServiceImpl {
	 ...
	 * 
	 * @param tag
	 *            The identification tag for logging.
	 */
	...
	public String[] getSenderIds(Context context) {
		return new String[] { CommonUtilities.getSenderId(context) };
	}
	...
	private void registerWithApp42(String regId, DeviceType deviceType) {
		...
		App42API.buildPushNotificationService().storeDeviceToken(
				App42API.getLoggedInUser(), regId, deviceType,
				new App42CallBack() {
					@Override
	...
	public void onUnregistered(Context context, String registrationId) {
		...
		CommonUtilities.displayMessage(context,
				context.getString(R.string.push_unregistered));
	}
	...
	public void onMessage(Context context, Intent intent) {
		...
		Log.i(mTag,
				"Received message " + intent.getExtras().getString("message"));

		int messages = CommonUtilities.storeMessage(context, message);
		String text = messages > 1 ? context.getString(
				R.string.push_message_many, messages) : context
	...
	private void generateNotification(Context context, String message,
			String text, String info, int id) {
